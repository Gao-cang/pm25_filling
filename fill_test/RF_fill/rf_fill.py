...
class RF_fill_test():
    ...
    def impute_test(self, all_data_csv):
        ...
        for station in station_columns:
            ...
            # 训练随机森林模型
            best_rf_model = self._preprocess_and_train_rf_model(data_to_fill, station)

            # 使用训练好的模型循环填补缺失值
            filled_data_for_station = self._fill_missing_values(data_to_fill, best_rf_model, station)
    ...
    def _preprocess_and_train_rf_model(self, df, station):
        """
        预处理数据并训练随机森林回归模型
        ...
        # 定义随机森林回归器和参数网格
        rf = RandomForestRegressor(random_state=42, n_estimators=200, max_depth=None, min_samples_split=2)
