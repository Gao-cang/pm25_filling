...
class XGB_fill_test():
    ...
    def impute_test(self, all_data_csv):
        ...
        for station in station_columns:
            ...
            data_to_fill = self._extract_data_for_station(station)

            # 训练xgb模型
            best_xgb_model = self._preprocess_and_train_xgb_model(data_to_fill, station)

            # 使用训练好的模型循环填补缺失值
            filled_data_for_station = self._fill_missing_values(data_to_fill, best_xgb_model, station)

            # 更新全局数据
            self.filled_data.loc[filled_data_for_station.index, station] = filled_data_for_station[station]

        return self.filled_data

    def _extract_data_for_station(self, station):
        """
        提取目标station及其相关特征
        ...
        columns_to_include = ['year', 'month', 'day', 'hour', 'is_weekday', 'season',
                             ...
                             'pressure', 'precipitation', station]
        return self.filled_data[columns_to_include]

    def _preprocess_and_train_xgb_model(self, df, station):
        """
        预处理数据并训练xgb回归模型
        ...
        """
        # 添加station_pre列（上一行值）
        df['station_pre'] = df[station].shift(1)

        # 分离已知和未知数据
        known_data = df[df[station].notnull()]
        unknown_data = df[df[station].isnull()]
        ...
        # 准备训练集
        X = known_data_to_train.drop(columns=[station])
        y = known_data_to_train[station]
        ...
        print(f"{station} Test set R^2: {r2}")
        print(f"{station} Test set MSE: {rmse}")
        print(f"{station} Test set MAE: {mae}")

        return xgb

    def _fill_missing_values(self, df, best_xgb_model, station):
        """
        循环填补缺失值
        :param df: 当前处理的DataFrame
        :param best_xgb_model: 训练好的xgb模型
        :param station: 当前处理的station列名
        :return: 填补后的DataFrame
        """
        df_filled = df.copy()
        ...
        while True:
            ...
            # 使用训练好的模型进行预测
            predictions = best_xgb_model.predict(X_predict)
