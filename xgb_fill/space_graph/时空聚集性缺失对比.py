...
# 读取时间区间文件
pos_df = pd.read_csv('pos_spatio_24.csv', parse_dates=['start_time', 'end_time'])
...
for i in range(1, 23):
    ...
    for idx, row in pos_df.iterrows():
        ...
        methods = ['1_forward', '2_backward', '3_bi', '4_knn', '5_linear']
        ...
        plt.ylabel(f'PM2.5 Value ({station})')
        ...
        # 保存图表
        filename = f"{station}/plot_{start_time.strftime('%Y%m%d_%H%M')}.png"
        plt.savefig(filename, dpi=150)
        plt.close()
    
    print(f"所有图表已生成到{station}目录")