...
def plot_clustered_barchart(df, type_algorithm):
    ...
    # 绘制MAE和RMSE（左侧轴），使用条纹和灰度
    # MAE: 横向条纹
    ax1.bar(x - width, df['mae'], width, label='MAE', 
            color='white', hatch='--', edgecolor='gray')
    
    # RMSE: 纵向条纹
    ax1.bar(x, df['rmse'], width, label='RMSE', 
            color='white', hatch='///', edgecolor='gray')
    ...
    # 绘制R²（右侧轴），使用浅灰色填充
    ax2.bar(x + width, df['r2'], width, label='R²', edgecolor='gray', 
            color='#A3A3A3', alpha=0.9)  # 浅灰色 (#D3D3D3)
